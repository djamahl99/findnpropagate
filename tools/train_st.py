...
from train_utils.train_utils import train_model_st
...
class WandbSummaryWriter:
    ...
    def add_scalar(
        ...
        if tag not in self.last_update:
            ...
            wandb.log(self.last_update, step=self.step)
            self.last_update = {}
...
def main():
    ...
    cfg.DATA_CONFIG['SKIP_NO_GT'] = False
    ...
    clip_bs = args.batch_size if clip_type == 'CROP' or clip_type == 'GLIP' else 1

    if use_clip:
        # inf dataset should have camera
        cam_cfg = cfg_from_yaml_file('cfgs/nuscenes_models/transfusion_lidar_demo.yaml',
                                        copy.deepcopy(cfg))
        INF_CONFIG = cam_cfg.DATA_CONFIG
    ...
    print('cfg.MODEL.DENSE_HEAD.SELF_TRAIN_PATH', cfg.MODEL.DENSE_HEAD.SELF_TRAIN_PATH)
    train_model_st(
        model,
        optimizer,
        train_loader,
